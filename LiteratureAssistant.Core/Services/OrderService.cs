    ...
    public class OrderService : Service<order>, IOrderService
    {
        ...
        public IEnumerable<order> ToEntities(IEnumerable<OrderViewModel> orderViewModels)
        {
            ...
            var result = orderViewModels.Select(i => new order
            {
                ...
                user = _userService.Find(string.IsNullOrEmpty(i.orderedByUserFullName) ? (i.orderedByUserId ?? 1) : (i.orderedByUserFullName == null ? null : (int?)_userService.Get(filter: j => (j.firstName + " " + j.lastName) == i.orderedByUserFullName).SingleOrDefault().userId)),

                user1 = _userService.Find(string.IsNullOrEmpty(i.orderedForUserFullName) ? (i.orderedForUserId ?? 0) : _userService.Get(filter: j => (j.firstName + " " + j.lastName) == i.orderedForUserFullName).SingleOrDefault().userId),
